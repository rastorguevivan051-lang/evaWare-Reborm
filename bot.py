"""
WindowClient Admin Bot
======================
pip install flask telebot requests
python bot.py
"""
...
def auth():
    ...
    if user is None:
        ...
        user.update({
            ...
            "version":  d.get("version",     user["version"]),
...
def fmt(u):
    return (
        ...
        f"🕐 <b>Последний вход:</b> <code>{u['last']}</code>\n"
...
def kb(hwid):
    ...
    m.add(
        InlineKeyboardButton("✅ ACTIVE", callback_data=f"s:active:{hwid}"),
...
def cmd_find(m):
    ...
    if len(parts) < 2:
        bot.send_message(ADMIN_ID, "Использование: /find имя"); return
...
def on_cb(c):
    ...
    if data.startswith("r:"):
        ...
        u = load().get(hwid)
        if u:
            try: bot.edit_message_text(fmt(u), cid, mid, reply_markup=kb(hwid))
            except: pass
    ...
    if data.startswith("s:"):
        ...
        db = load()
        if hwid in db:
            db[hwid]["status"] = status
            save(db)
            u = db[hwid]
            try: bot.edit_message_text(fmt(u), cid, mid, reply_markup=kb(hwid))
            except: pass
            labels = {"active":"✅ ACTIVE","frozen":"❄️ FROZEN","banned":"🚫 BANNED"}
            bot.answer_callback_query(c.id, labels.get(status,"✅"))
        else:
...
def send_list(cid, mid, page):
    ...
    for u in chunk:
        ...
        mk.add(InlineKeyboardButton(f"{e} UID {u['uid']} — {u['name']}",
                                    callback_data=f"u:{u['hwid']}"))
    nav = []
...
if __name__ == "__main__":
    print(f"[*] WindowClient Bot запущен на порту {PORT}")
    print(f"[*] Для теста локально — запусти ngrok http {PORT} с VPN")
    print(f"[*] На сервере — просто python bot.py, ngrok не нужен")
